...
main() {
	...
	$SUPRAPACK_PATH install $path_build/*.suprapack --yes
	source "$HOME/.suprapack_profile"
...
check_dependencies() {
	...
	if [ -n "$missing_commands" ]; then
		...
		if [ $OS = "Linux" ]; then
			...
			printf '\033[95;1mmacOS: \033[95mbrew install pygobject3 gobject-introspection vala pkg-config gcc glib-networking gtk4 curl wget git ccls openssh openssl meson ninja fakeroot\n'
		fi
		printf '\033[0m'
		exit 1
	fi
